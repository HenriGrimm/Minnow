...
/**
 * JSON is copied into extraResources for skill install. The .mjs twin stays in
 * app.asar so packaged `harness-registry.mjs` can import it.
 *
 * @param {string} targetDir
 * @param {string[]} names
 */
function writeHarnessCommandArtifacts(targetDir, names) {
  fs.writeFileSync(
    path.join(targetDir, 'harness-commands.json'),
    `${JSON.stringify(names, null, 2)}\n`,
    'utf8',
  );
  const body = names.map((name) => `  ${JSON.stringify(name)},`).join('\n');
  fs.writeFileSync(
    path.join(targetDir, 'harness-commands.mjs'),
    `/**
...
export function applyMinnowImpeccablePatches(targetDir) {
  ...
  if (fs.existsSync(pinPath)) {
    ...
    writeHarnessCommandArtifacts(targetDir, harnessNames);
  }
