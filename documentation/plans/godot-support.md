...
3. **Editor ownership:** connect to a running project editor when configured. Only launch a managed editor/headless LSP instance after checking that no editor for that project is already running, and never silently start a duplicate. Record which process Minnow owns and stop only that process. Use loopback sockets, configurable or allocated ports, bounded retries, and socket/protocol handshakes. A stray Godot instance on a common port is not proof it owns this project. Earlier Godot issue reports document [duplicate project editors](https://github.com/godotengine/godot/issues/58723) and [temporary-file collisions between instances](https://github.com/godotengine/godot/issues/16679); validate the behavior on supported Godot 4 builds before enabling managed mode.
...
- Watch or compare disk revisions for open Godot source tabs: clean buffers reload on external save, dirty buffers show a compare/keep/reload choice, and Save refuses to overwrite an unseen disk change. Coalesce import/cache churn; never watch all of `.godot/` into the renderer.
