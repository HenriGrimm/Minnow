...
Minnow is a **Vite + TypeScript SPA** plus a **Node tool server** (`server.js`) and an **Electron shell** (Minnow Shell). It is a local-first **development workspace** — editor, terminal, git, issues, planning, agents, research, and local model hosting in one app — running on LM Studio and other OpenAI-compatible providers.
...
- **Four composer modes** — General, Build, Plan (no-destructive guard), Debug — plus **Orchestrate** (opens the Boards surface from the sidebar, not a chat; not in the composer picker), **Super Plan**, and **Onboarding** as non-composer modes. Persisted `'desktop'` and `'email'` chats remap to **General** via `normalizeModeId`. Defined in [`src/chat/modes/registry.ts`](src/chat/modes/registry.ts); prompts in [`src/chat/prompts/modes/`](src/chat/prompts/modes/). Reef mode was removed (MIN-473) — do not reintroduce it.
...
- **Minnow apps — released:** **Code** (the primary surface), Research, Models, Brain, Scheduler, **Issues**, Settings — all `core` (not user-disableable). Legacy `#/app/chat` redirects to `#/app/code/chat`. **Hidden** (`releaseState: 'hidden'`, MIN-471): Compare, Bench, Experts — code stays in tree but is omitted from dock, onboarding, Settings, routes, notifications, and `launch_minnow_app`. Registry: [`src/os/app-registry.ts`](src/os/app-registry.ts).
