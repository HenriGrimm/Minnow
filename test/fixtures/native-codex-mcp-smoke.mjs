...
const server = createServer(async (req, res) => {
  ...
  const item = first && name
    ...
    : { type: 'message', id: 'msg_smoke', role: 'assistant', content: [{ type: 'output_text', text: name ? 'TOOL_RESULT_RECEIVED' : 'MCP_TOOL_MISSING', annotations: [] }], status: 'completed' };
  const response = { id: `resp_smoke_${requests.length}`, object: 'response', status: 'completed', output: [item], usage: { input_tokens: 1, output_tokens: 1, total_tokens: 2 } };
  res.writeHead(200, { 'content-type': 'text/event-stream' });
  for (const value of [
    { type: 'response.created', response: { ...response, status: 'in_progress', output: [] } },
    { type: 'response.output_item.added', output_index: 0, item: { ...item, status: 'in_progress', ...(first && name ? { arguments: '' } : {}) } },
    ...(first && name ? [
    ...
    ] : [{ type: 'response.output_text.delta', item_id: item.id, output_index: 0, content_index: 0, delta: name ? 'TOOL_RESULT_RECEIVED' : 'MCP_TOOL_MISSING' }]),
...
try {
  ...
  assert.ok(requests.some(request => request.tools?.some(tool => tool.name === 'mcp__minnow' && tool.tools?.some(nested => nested.name === 'ping'))), 'Native Codex must receive the Minnow MCP tool');
  assert.equal(handoff?.function.name, 'ping', 'Native Codex must hand off the requested tool');
  assert.equal(requests.length, 2, 'Codex must continue in the same process after the Minnow tool result');
  assert.match(JSON.stringify(requests[1].input), /Actual Minnow tool result/, 'Codex must send the real tool result to its next model request');
  ...
  assert.ok(requests.every(request => request.tools?.every(tool => tool.name?.startsWith('mcp__minnow') || discoveryHelpers.includes(tool.name))), 'Native execution tools must remain disabled');
} finally {
  clearTimeout(timer);
  await child?.stop();
  await bridge?.close();
