...
`git_diff` scope check → static integration review → run the Test spec and affected tests; add diagnostics/typecheck or integration checks proportional to risk. Reserve the full ladder for final integration unless explicitly required. For UI acceptance criteria, inspect the implementation and report any behavior that cannot be verified without a browser. Do not edit application code. Never `background: true` for typecheck, lint, test, or build.
